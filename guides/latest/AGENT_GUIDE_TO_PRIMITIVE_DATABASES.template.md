...
{{ example: databases/db-execute-operation }}

### List / get databases

{{ example: databases/db-list-get }}
...
- `query` with an inclusion `projection` → `QueryResult<Pick<Model, projectedFields | "id">>` (`id` is always returned). A static all-exclusion `projection` over known schema fields → `QueryResult<Omit<Model, excludedFields>>` — the server omits those fields from the result, so reading an excluded field is a compile error; `id`/`type` are always returned and can't be excluded, so they never enter the omit set. Dynamic (`$params.*`), nested-key, mixed static-and-dynamic, or unknown-key projections fall back to `QueryResult<Partial<Model>>` — as does an exclusion set that is only `id`/`type` (empty after filtering). No `[models.*]` schema → `QueryResult<Record<string, unknown>>`.
...
**Typed subscription calls.** When the type declares `[[subscriptions]]` blocks, the file also emits a subscriptions factory named `camelCase(databaseType) + "Subscriptions"` (`portfolio` → `portfolioSubscriptions`): one method per subscription, keyed by the subscription key (non-identifier keys become quoted). Each method takes `SubscribeOptions<T>` — its `onChange` receives a change payload whose `changes[].data` / `changes[].previousData` are typed `T` instead of `any` — and returns the `unsub()` function:

```ts
const subs = portfolioSubscriptions(client, databaseId);
const unsub = subs.openAccounts({
  onChange: (event) => {
    for (const change of event.changes) {
      change.data;          // Account | null (typed from the subscription's modelName)
    }
  },
});
```

`T` is the record interface of the subscription's declared `modelName`; a subscription that declares `select` narrows it to `Pick<Account, …selectedFields>`. Codegen **throws** if a subscription has no `modelName`, names a model with no `[models.*]` schema, or `select`s a field the model doesn't declare — the shape can't be typed, so it fails the run rather than widening to `unknown`. The subscriptions factory shares the same import-free `OpsClient` interface as the ops factory; when subscriptions exist, `OpsClient` gains a `subscribe<T>(databaseId, subscriptionKey, options)` member. See [Real-Time Subscriptions](#real-time-subscriptions) for the subscription API, TOML shape, and what each `op` puts in `data`.
{{/lang}}
...
`fromWorkflow(key)` authorizes the **workflow**, not the caller inside it. In a `runAs: "caller"` run the step still executes as the invoking user, so gating an op with `fromWorkflow('this')` and then passing it a caller-supplied key that selects another user's row reopens the access gap one step removed — the op's `params`/`definition` still need to scope to the caller (`$user.userId`, `params.userId == user.userId`), the same as an ungated op. See [Authorizing a caller workflow](AGENT_GUIDE_TO_PRIMITIVE_WORKFLOWS.md#authorizing-a-caller-workflow) in the Workflows guide.
...
{{ example: databases/db-execute-paginated }}
...
{{ example: databases/db-execute-timing }}
...
If you want the server to enforce that registered operations only reference fields you've declared, declare those same models *inside the database type config* (`config/database-type-configs/<type>.toml`) using `[models.<Name>.fields.<field>]` blocks. The CLI config push extracts that subtree and stores it as the type's `schema`, which the op-edit and schema-edit gates check against — see [Schema gate](#schema-gate).
...
{{ example: databases/db-group-permissions }}
...
{{#lang ts}}- `subscribe()` returns an `unsub()` function — there is **no** event-emitter API (`.on()` / `.unsubscribe()`). Call `unsub()` on teardown or you leak `ConnectionMapping` rows and dead callbacks.{{/lang}}{{#lang swift}}- `subscribe()` returns an `EventSubscription` — there is **no** event-emitter API (`.on()` / `.unsubscribe()`). Hold it for as long as you want changes and `cancel()` it on teardown; releasing the handle unsubscribes, and holding one past teardown leaks a `ConnectionMapping` row and a live callback.{{/lang}}
...
6. {{#lang ts}}**`unsub()` leaks if not called.** Each `subscribe()` returns an `unsub` function. Call it on view teardown or you accumulate `ConnectionMapping` rows and dead callbacks.{{/lang}}{{#lang swift}}**Store the `EventSubscription` for the view's lifetime.** Releasing the handle unsubscribes, so a subscription you don't hold stops delivering right away; one you hold past teardown keeps a `ConnectionMapping` row and a live callback until you `cancel()` it.{{/lang}}
...
{{#lang ts}}- Forgetting to call the returned `unsub()`. Leaks `ConnectionMapping` rows and registry entries.
- Subscribing on every render. Subscribe once per view, unsub on unmount.{{/lang}}{{#lang swift}}- Discarding the returned `EventSubscription` (assigning it to `_`, or letting it go out of scope). The subscription ends the moment the handle is released.
- Subscribing on every render. Subscribe once per view and hold the handle; `cancel()` it on teardown.{{/lang}}
...
{{ example: databases/db-user-scoped-execute }}
