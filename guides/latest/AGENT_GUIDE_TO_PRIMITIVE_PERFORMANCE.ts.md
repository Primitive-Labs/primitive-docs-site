...
```typescript
  const result = await client.functions.invoke<{
    ...
    accounts: unknown[];
    holdings: unknown[];
  }>("dashboard-bundle", { input: { databaseId } });

  const groups = result.output?.groups ?? [];
  const accounts = result.output?.accounts ?? [];
  const holdings = result.output?.holdings ?? [];
```
...
```typescript
  const result = await client.functions.invoke<{ items: TaskAttrs[] }>(
    "list-all-tasks",
    { input: { databaseId } },
  );
  const byCategory = new Map<string, TaskAttrs[]>();
  for (const task of result.output?.items ?? []) {
...
```typescript
  const [bundle, memberships] = await Promise.all([
    client.functions.invoke("dashboard-bundle", { input: { databaseId } }),
    client.groups.listUserMemberships(userId),
  ]);
```
