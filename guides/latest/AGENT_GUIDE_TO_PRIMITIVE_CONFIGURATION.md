...
**The preflight is the only all-or-nothing stage.** `config push` runs a local pass over the tree before the first mutating call, and any error in it aborts the run with nothing applied (`Aborting push: N TOML validation error(s) — no changes were applied.`). What that pass checks is a named list, not "everything": unrecognized keys and declared types on every config type, the workflow lints, a function's grant grammar, a function's `entry`, the function trigger rules, and a webhook trigger's signing-secret requirement and reference shape. Those are the SERVER's own rules — the CLI reaches them through the generated surface artifact (`cli/src/lib/generated-config-surfaces.ts`, vendored from `src/config-surface/` and held fresh by a build check), so a preflight refusal is the same sentence the server would have produced. Cross-field constraints inside one object are decidable from the file alone because the state a push writes is file-derived: a workflow update sends the whole `[workflow]` table, and a function's version envelope carries the whole authored TOML.
