...
- **Upload** `tags: string[]` is the only extra metadata — there is no arbitrary `metadata` field. Result: `{ blobId, bucketId, filename, contentType, numBytes, sha256, tags, createdBy }`.{{#lang ts}} Accepts `data: ArrayBuffer | Uint8Array | Blob | string`; a `File` works (it's a `Blob`).{{/lang}}{{#lang swift}} Accepts `data: Data`.{{/lang}}
...
- **Download** returns the blob bytes via an authenticated request.{{#lang ts}} The result is an `ArrayBuffer`.{{/lang}}{{#lang swift}} The result is `Data`.{{/lang}}
