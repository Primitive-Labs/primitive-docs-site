...
| `409 WORKFLOW_ARCHIVED` | The workflow was retired by a delete. `enable` will not bring it back: hard-delete the archived holder (console purge, or remove its file and run `primitive config push --prune`), then re-add the file and push |
