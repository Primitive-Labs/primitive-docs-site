...
```swift
  ...
  let groups = result.output?["groups"]?.arrayValue ?? []
  let accounts = result.output?["accounts"]?.arrayValue ?? []
  let holdings = result.output?["holdings"]?.arrayValue ?? []
```
...
```swift
  ...
  for task in result.output?["items"]?.arrayValue ?? [] {
...
```swift
  async let bundle: FunctionResult<JSONValue> = client.functions.invoke(
    "dashboard-bundle",
    input: ["databaseId": databaseId]
  )
  async let memberships = client.groups.listUserMemberships(userId: userId)

  let (b, m) = try await (bundle, memberships)
```
