...
| Server function | `access` on `[function]` | identity context | Evaluated on every HTTP invocation, and on a `workflow.call` step whose `workflowKey` names a function rather than a workflow — there the function's `access` is evaluated against the **parent run's caller** for a caller-mode parent, while a system parent skips it (mirroring the child-workflow rule above). admin/owner bypass. **No rule → denied**; push refuses code for a function with no gate. Denial: `403 { errorCode: "FUNCTION_ACCESS_DENIED" }`. The gate IS the authorization: inside, the code acts as the system. |
...
Resolve the provider id server-side from the authenticated user — never accept a client-supplied `customer_id`, and the same for any internal id a workflow step would otherwise take from caller input (see External identifiers, above).
