...
| Workflow | `accessRule` on `[workflow]` | identity context + `fromWorkflow()` / `fromWorkflow(key)` for a `workflow.call` step | Evaluated on `workflows.start()`, `runSync()` and `workflow.call`; NOT on webhook/cron triggers. admin/owner bypass. **No rule → denied** (#2652), existing rows included; `"true"` restores open access. Required at create on a caller workflow; a `runAs: "system"` workflow rejects any rule. A child called by a known parent can name it — `fromWorkflow('<parent-key>')` — and a `workflow.call` from a SYSTEM parent skips the child's rule entirely (#1078). Denial: `403 { errorCode: "WORKFLOW_ACCESS_DENIED" }`. |
| Prompt | `accessRule` on `[prompt]` | identity context + `fromWorkflow()` / `fromWorkflow(key)` for a `prompt.execute` step | Evaluated on `client.prompts.execute()` and the workflow `prompt.execute` step (both run modes). admin/owner bypass. **No rule → denied** (#2652); required at create on every surface. Denial: `403 { errorCode: "PROMPT_ACCESS_DENIED" }`, before any model call, credential read or `prompt.executed` event. A `runAs: "system"` run binds no user, so authorize it with `fromWorkflow('<key>')`. |
| Integration | `accessRule` on `[integration]` | identity context + `fromWorkflow()` / `fromWorkflow(key)` for an `integration.call` step | Evaluated on the proxy route and the `integration.call` step. admin/owner bypass. **No rule → denied** (#2631); required at create. Denial: `403 { errorCode: "INTEGRATION_ACCESS_DENIED" }`. |
| Named locks, direct LLM/Gemini proxy routes | — | — | No per-resource rule exists: any app member may call them. Tracked in #2686 / #2687; prefer a managed prompt (which is gated) over the LLM proxy routes. |
