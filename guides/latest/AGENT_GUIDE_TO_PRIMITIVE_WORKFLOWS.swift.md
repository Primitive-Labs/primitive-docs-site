...
The `workflowStatus` event and `getStatus` report the same vocabulary — see [Run status vocabulary](#run-status-vocabulary). The event fires only on a terminal transition, so its `status` is always `completed`, `failed`, or `terminated`.
...
`client.nextEvent(_:timeout:where:)` is the one-shot form: it awaits the first event the predicate accepts, throws `JsBaoError` with code `.unavailable` on timeout (default 10 seconds), and throws `CancellationError` if the calling task is cancelled. It replaces the deprecated free `waitForEvent(emitter:event:timeout:predicate:)`.
...
let result = try await client.workflows.waitFor(
    runId: runId,
    options: WaitForWorkflowOptions(timeoutMs: 60000)
)
// result: WaitForWorkflowResult { status: String, output: JSONValue?, error: String? }
```

`waitFor(runId:as:options:)` decodes `output` into a `Decodable` type instead of a raw `JSONValue`. Pass a `timeoutMs` of `0` (or any non-positive value) to disable the timeout — the call and its listeners then live until the run terminates, so only do that for runs you know will end. Cancelling the awaiting `Task` tears the wait down and throws `CancellationError`.
