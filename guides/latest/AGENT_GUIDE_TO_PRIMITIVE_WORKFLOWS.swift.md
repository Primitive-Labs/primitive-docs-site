...
**The key may name a server function.** `workflowKey` names a key, not a type: a live workflow always wins it, but when the key names a server function — because no workflow holds it, or the workflow holding it was archived and the function took the key over — the function runs instead and its output becomes the step's `output`. For a caller-mode parent the callee function's own `access` gate is evaluated against the parent run's caller before dispatch (a denial fails the step with `FUNCTION_ACCESS_DENIED`); a system parent skips it, exactly as it skips a child workflow's `accessRule`. No separate run is recorded — the parent's step run is the record. This is the migration bridge and it goes one way: a function never calls a workflow. It retires with the workflow engine in project phase 7. See [Server Functions](AGENT_GUIDE_TO_PRIMITIVE_SERVER_FUNCTIONS.md).
...
```bash
# Sync (recommended for everything; sync dir auto-resolves to primitive/<env>/)
...
The command reads `workflows/*.toml` from the selected environment's config directory, and from nowhere else — `--env <name>` selects a different environment; no flag relocates the directory — and emits **one `<key>.generated.swift` per workflow** (default output `<config-dir>/workflows/generated/`; stale generated files for removed workflows are cleaned up on full runs). Reserved `__internal.*` workflows are skipped; malformed schema JSON fails the command. A single `[workflow-key]` argument matches the TOML file stem and generates just that file.
...
In a repo shared by several clients — an iOS client paired with a web client, say — the workflow TOML lives once at the repository root, in `primitive/<env>/workflows/`. Run codegen from the client directory and say where the output goes; the walk-up finds the same project either way:
